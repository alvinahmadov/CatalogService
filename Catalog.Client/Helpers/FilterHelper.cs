...
namespace Catalog.Client
{
    public static class FilterHelper
    {
        ...
        public static DbQuery<T> Filter<T>(IQueryable<T> queryable, FilterDescriptorCollection filters) // where T : ISavableObject
        {
            if (filters != null && filters.Count > 0)
            {

                Expression predicate = BuildFilter(queryable.ElementType, filters);
                var exp = Expression.Call(typeof(Queryable), "Where", new[] { queryable.ElementType }, queryable.Expression, Expression.Quote(predicate));
                queryable = queryable.Provider.CreateQuery(exp) as DataServiceQuery<T>;
            }

            return queryable as DbQuery<T>;
        }
